...
| 10 | *absent* | `CLOUDINARY_URL` — **deliberately still absent** | The one matrix variable CI should not carry. It is conditionally required: `STORAGE_PROVIDER=s3` needs the `AWS_*` four and `STORAGE_PROVIDER=cloudinary` needs this one, and no environment needs both. Recorded here so its absence reads as a decision rather than the eleventh oversight |
...
> [!NOTE]
> **Every command above is offline on purpose.** `prisma validate` reads `DATABASE_URL` but never dials it, and `db seed` against the current 0-byte stub opens no connection either, so `.env.example`'s placeholder credentials are sufficient and no Postgres is required for Day 0. Anything that connects — `migrate dev`, `migrate deploy`, `db seed` against a real script — belongs to Day 1 after `docker compose up`. Attempting `migrate deploy` here exits 1 with `P1001 Can't reach database server`, which is the environment answering correctly, not a Day 0 failure.
