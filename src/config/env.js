...
const envSchema = z.object({
  ...
  if (data.JWT_SECRET && data.JWT_REFRESH_SECRET && data.JWT_SECRET === data.JWT_REFRESH_SECRET) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'JWT_SECRET and JWT_REFRESH_SECRET must be distinct — one key for both token classes lets a refresh token authenticate as an access token (TRD §7)', path: ['JWT_REFRESH_SECRET'] });
  }

  if (data.STORAGE_PROVIDER === 's3') {
    if (!data.AWS_ACCESS_KEY_ID) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'AWS_ACCESS_KEY_ID is required for s3', path: ['AWS_ACCESS_KEY_ID'] });
    if (!data.AWS_SECRET_ACCESS_KEY) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'AWS_SECRET_ACCESS_KEY is required for s3', path: ['AWS_SECRET_ACCESS_KEY'] });
    if (!data.AWS_REGION) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'AWS_REGION is required for s3', path: ['AWS_REGION'] });
    if (!data.AWS_S3_BUCKET) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'AWS_S3_BUCKET is required for s3', path: ['AWS_S3_BUCKET'] });
  } else if (data.STORAGE_PROVIDER === 'cloudinary') {
    if (!data.CLOUDINARY_URL) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'CLOUDINARY_URL is required for cloudinary', path: ['CLOUDINARY_URL'] });
  }

  if (data.NODE_ENV === 'test') {
    if (!data.DATABASE_URL_TEST) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'DATABASE_URL_TEST is required in test environment', path: ['DATABASE_URL_TEST'] });
    if (!data.REDIS_URL_TEST) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'REDIS_URL_TEST is required in test environment', path: ['REDIS_URL_TEST'] });
  }
});
